...
This is a **fork of [goharbor/harbor](https://github.com/goharbor/harbor)** maintained by isityael.
...
- **`main` stays synced with upstream** — never commit directly to `main`.
  `isityael/v2.15` is the patched branch for the v2.15 series.
...
git checkout -b isityael/v2.15-ga v2.15.0
...
git push origin isityael/v2.15 --tags
...
- **Secrets used:** `harbor_username`, `harbor_password`, `mac_builder_ssh_key`,
  `cosign_key`, `cosign_password`, `notation_key`, `notation_cert` (org-level on isityael)

## Consumer

The infra repo (`isityael/infra`) overrides image repos/tags in
`apps/user/harbor/values.yaml` to point to our fork images.
The harbor-helm chart (`isityael/harbor-helm`) is a separate fork.
